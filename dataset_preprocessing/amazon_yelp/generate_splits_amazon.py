...
def generate_user_splits(
    data_dir, reviews_df, min_size_per_user, train_size, eval_size, seed
):
    # mark duplicates
    ...
    reviews_df["duplicate"] = (
        reviews_df["reviewText"].isin(duplicated_text)
    ) | duplicated_within_user
    # mark html candidates
    reviews_df["contains_html"] = reviews_df["reviewText"].apply(
...
def generate_users_baseline_splits(
    data_dir, reviews_df, reviewer_id, seed, user_split_name="user"
):
    # seed
    np.random.seed(seed)
    # sizes
    ...
    splits[
        (
            (orig_splits_df["split"] == OOD_TEST)
            & (reviews_df["reviewerID"] == reviewer_id)
        ).values
    ] = TEST
    # train
...
def main():
    ...
    df = pd.read_csv(
        reviews_path(data_dir(args.root_dir)),
        dtype={
            "reviewerID": str,
            "asin": str,
            "reviewTime": str,
    ...
    for train_categories in train_categories_list:
        split_name = (
            ",".join([category.lower() for category in train_categories])
            + "_generalization"
        )
        generate_fixed_group_splits(
            data_dir=data_dir(args.root_dir),
