...
def load_business_data(data_dir):
    ...
    for k in keys:
        df[k] = []
    with open(
        os.path.join(raw_data_dir(data_dir), "yelp_academic_dataset_business.json"), "r"
    ) as f:
...
def compute_token_length(data_dir):
    ...
    token_counts = []
    with open(
        os.path.join(raw_data_dir(data_dir), "yelp_academic_dataset_review.json"), "r"
    ) as f:
        text_list = []
        for i, line in enumerate(f):
            if i % 100000 == 0:
...
def process_reviews(data_dir):
    ...
    with open(reviews_path(data_dir), "w") as f:
        ...
        for i, review in enumerate(
            parse(
                os.path.join(
                    raw_data_dir(data_dir), "yelp_academic_dataset_review.json"
                )
            )
        ):
    ...
    duplicated_text = df_deduplicated_within_user[
        df_deduplicated_within_user["text"]
...
def main():
    ...
    for dirpath in [
        splits_dir(data_dir(args.root_dir)),
        preprocessing_dir(data_dir(args.root_dir)),
    ]:
