...
def generate_time_splits(
    ...
    for year in range(min(reviews_df[year_field]), max(reviews_df[year_field]) + 1):
        ...
        eval_indices = np.random.choice(year_indices, curr_eval_size * 2, replace=False)
    ...
    # train
    (train_year_indices,) = np.where(
        np.logical_and(reviews_df[year_field] <= year_threshold, splits == -1)
    )
    train_indices = np.random.choice(train_year_indices, train_size, replace=False)
    splits[train_indices] = TRAIN
    (baseline_train_year_indices,) = np.where(
        np.logical_and(reviews_df[year_field] > year_threshold, splits == -1)
    )  # require spits!=-1 to reserve ID_VAL, ID_TEST too
    baseline_train_indices = np.random.choice(
        baseline_train_year_indices, train_size, replace=False
    )
    ...
    pd.DataFrame({"split": baseline_splits}).to_csv(
        splits_path(data_dir, "time_baseline"), index=False
    )
...
def generate_group_splits(
    ...
    if select_column is not None:
        group_counts = (
            reviews_df[reviews_df[select_column]][group_field]
            .value_counts()
            .reset_index()
        )
    else:
    ...
    groups = group_counts[group_counts["count"] >= min_size_per_group][
        group_field
    ].values
    ...
    while cumulative_train_size < train_size and group_idx < len(groups):
        ...
        if select_column is not None:
            (curr_group_indices,) = np.where(
                (reviews_df[group_field] == curr_group) & reviews_df[select_column]
            )
        else:
    ...
    for split in (OOD_VAL, OOD_TEST):
        ...
        while cumulative_eval_size < eval_size and group_idx < len(groups):
            ...
            if select_column is not None:
                ...
                (curr_group_indices,) = np.where(
                    (reviews_df[group_field] == curr_group)
                )
            data_indices = np.random.choice(
                curr_group_indices, eval_size_per_group, replace=False
            )
...
def generate_fixed_group_splits(
    data_dir,
    reviews_df,
    group_field,
    train_groups,
    ...
    for group in unique_groups:
        ...
        eval_indices = np.random.choice(
            group_indices, curr_eval_size * 2, replace=False
        )
    ...
    if train_groups is None:  # subpopulation shift
        ...
        (train_group_indices,) = np.where(
            np.logical_and(
                reviews_df[group_field].isin(set(train_groups)).ravel(), splits == -1
            )
        )
