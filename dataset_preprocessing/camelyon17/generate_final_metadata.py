...
def generate_final_metadata(output_root):
    df = pd.read_csv(
        os.path.join(output_root, "all_patch_coords.csv"),
        index_col=0,
    ...
    for center in range(num_centers):
        ...
        print(
            f"  Num non-tumor: {min(num_tumor, num_non_tumor)} out of {num_non_tumor} ({min(num_tumor, num_non_tumor) / num_non_tumor * 100:.1f}%)"
        )
    ...
    val_indices = list(
        np.random.choice(
            all_indices, size=int(val_frac * len(all_indices)), replace=False
        )
    )
    ...
    for center in range(num_centers):
        ...
        print(f"Center {center}")
        print(
            f"  {num_tumor} / {num_tumor + num_non_tumor} ({num_tumor / (num_tumor + num_non_tumor) * 100:.1f}%) tumor"
        )
