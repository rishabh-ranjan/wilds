...
def construct_splits(df):
    ...
    val_articles = np.random.choice(
        list(train_articles), size=n_val_articles, replace=False
    )
    ...
    new_test_articles = np.random.choice(
        list(train_articles), size=n_new_test_articles, replace=False
    )
    ...
    assert n_train == len(train_df) + len(val_df) + np.sum(
        df["article_id"].isin(new_test_articles)
    )
    assert n_test == len(test_df) - np.sum(df["article_id"].isin(new_test_articles))
    assert n_train_articles == len(train_articles) + len(val_articles) + len(
        new_test_articles
    )
