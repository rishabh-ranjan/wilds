...
def load_unlabeled_df(root):
    ...
    df = df.dropna(
        axis=0, how="any", subset=["id", "comment_text", "article_id"]
...
def main(args):
    ...
    def output_split_sizes(df):
        ...
        print(f'Unlabeled size: {len(df[df["split"] == UNLABELED])}')
        print("-" * 50)
        print("\n")
    ...
    joint[joint["split"] == UNLABELED].to_csv(
        f"{args.root}/unlabeled_data_with_identities.csv", index=True
    )
    print("Done.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create unlabeled splits for CivilComments."
    )
    parser.add_argument(
        "--root",
        type=str,
