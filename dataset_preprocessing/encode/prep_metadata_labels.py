...
def write_label_bigwigs(
    ...
    train_regions_labeled = pd.read_csv(
        os.path.join(_data_dir, "labels/{}.{}".format(tf_name, train_suffix)), sep="\t"
    )
    val_regions_labeled = pd.read_csv(
        os.path.join(_data_dir, "labels/{}.{}".format(tf_name, val_suffix)), sep="\t"
    )
    ...
    _unsorted_dir = _data_dir + "labels/{}/{}_posamb.bed".format(tf_name, tf_name)
    _sorted_dir = _unsorted_dir.replace(
        "{}_posamb".format(tf_name), "{}_posamb.sorted".format(tf_name)
    )
    _metadata_df.to_csv(_unsorted_dir, sep="\t", header=False, index=False)
    print(time.time() - itime)
    ...
    mdf_posamb = pd.read_csv(
        ...
        header=None,
        index_col=None,
        names=["chr", "start", "stop", "y", "celltype"],
    )
...
def write_metadata_products(
    ...
    for ct in celltypes:
        ...
        if (
            posamb_only
        ):  # Retrieve only the windows containing positively/ambiguously labeled bins
            df = mdf_posamb[mdf_posamb["celltype"] == ct]
            df["window_start"] = stride * (df["start"] // stride)
            uniq_windows = np.unique(
                ["{}:{}".format(x[0], x[1]) for x in zip(df["chr"], df["window_start"])]
            )
            ...
            for chrID in bw.chroms():
                ...
                for startc in np.arange(
                    int(stride / 2), chromsize - (2 * stride), stride
                ):
                    u_end = startc + stride
                    if u_end > chromsize:
                        break
        ...
        celltype_mdta_df = pd.DataFrame(
            df_construction, columns=["chr", "start", "stop"]
        )
    ...
    np.save(
        _data_dir + "labels/{}/{}".format(tf_name, y_arr_filename),
        np.vstack(celltype_labels),
    )
...
if __name__ == "__main__":
    ...
    for tf_name in tfs_to_celltypes:
        ...
        write_label_bigwigs(
            ["liver"],
            train_suffix="train_wc.labels.tsv.gz",
            val_suffix="test.labels.tsv.gz",
            tf_name=tf_name,
        )
