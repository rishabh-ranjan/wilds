...
def create_split(data_dir, seed):
    ...
    df_metadata = pd.DataFrame(
        {
            ...
            "frame_num": [
                item["frame_num"] for item in data["images"]
            ],  # this attribute is not used
            "seq_id": [
                item["seq_id"] for item in data["images"]
            ],  # this attribute is not used
        }
    )
    ...
    seq_ids_that_span_across_days = (
        nunique_dates[nunique_dates.values > 1].reset_index()["seq_id"].values
    )
    ...
    train_locations, val_trans_locations = (
        locations[:n_train_locations],
        locations[n_train_locations : (n_train_locations + n_val_locations)],
    )
    test_trans_locations = locations[(n_train_locations + n_val_locations) :]

    remaining_df, val_trans_df = (
        df[df["location"].isin(train_locations)],
        df[df["location"].isin(val_trans_locations)],
    )
    ...
    train_dates, val_cis_dates = (
        unique_dates[:n_train_dates],
        unique_dates[n_train_dates : (n_train_dates + n_val_dates)],
    )
    ...
    locs_to_be_moved = []
    locs_to_be_moved.extend(
        list(set(val_cis_df["location"]) - set(train_df["location"]))
    )
    locs_to_be_moved.extend(
        list(set(test_cis_df["location"]) - set(train_df["location"]))
    )
    ...
    for seq_id in seq_ids_that_span_across_days:
        ...
        for split_df in [train_df, val_cis_df, test_cis_df]:
            ...
            assert (
                n_splits == 1
            ), "Each sequence should only be in one split. Please move manually"

    # Reset index
    train_df.reset_index(inplace=True, drop=True), val_cis_df.reset_index(
        inplace=True, drop=True
    ), val_trans_df.reset_index(inplace=True, drop=True)
    test_cis_df.reset_index(inplace=True, drop=True), test_trans_df.reset_index(
        inplace=True, drop=True
    )
    ...
    category_to_label = dict(
        [(i, j) for i, j in zip(unique_categories, range(n_classes))]
    )
    ...
    # Save
    df = df.sort_values(
        ["split", "location_remapped", "sequence_remapped", "datetime"]
