...
class Batcher:
    ...
    def get_batch(self):
        ...
        if self.shuffle:
            ...
            dataset = dataset.apply(
                tf.contrib.data.parallel_interleave(
                    lambda file_path: tf.data.TFRecordDataset(
        ...
        dataset = dataset.map(
            self.process_tfrecords, num_parallel_calls=self.num_threads
        )
    ...
    def process_tfrecords(self, example_proto):
        ...
        keys_to_features = {}
        for band in bands:
            keys_to_features[band] = tf.FixedLenFeature(
                shape=[255**2], dtype=tf.float32
            )
            ...
            for band in bands:
                ...
                if self.normalize:
                    if band == "NIGHTLIGHTS":
                        ex[band] = tf.cond(
                            year < 2012,  # true = DMSP
                            true_fn=lambda: (ex[band] - means["DMSP"])
                            / std_devs["DMSP"],
                            false_fn=lambda: (ex[band] - means["VIIRS"])
                            / std_devs["VIIRS"],
                        )
                    else:
