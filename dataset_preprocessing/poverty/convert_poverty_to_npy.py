...
if __name__ == "__main__":
    tfrecord_paths = np.asarray(
        batcher.get_tfrecord_paths(dataset=DATASET, split="all")
    )
    ...
    for i in tqdm(range(num_batches)):
        ...
        for path, loc, year, nl_mean, nl_center in zip(
            batch_paths, img_batch["locs"], img_batch["years"], nl_means, nl_centers
        ):
        ...
        if len(imgs) > (10000 // 32):
            imgs = np.concatenate(imgs, axis=0)
            np.save(
                f"/u/scr/nlp/dro/poverty/unlabeled_landsat_poverty_imgs_{counter}.npy",
                imgs,
            )
            counter += 1
            imgs = []
    if len(imgs) > 0:
        imgs = np.concatenate(imgs, axis=0)
        np.save(
            f"/u/scr/nlp/dro/poverty/unlabeled_landsat_poverty_imgs_{counter}.npy", imgs
        )
