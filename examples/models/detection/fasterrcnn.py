...
def batch_concat_box_prediction_layers(box_cls, box_regression):
    ...
    for batch_idx in range(batch_size):
        element_box_cls = [
            torch.unsqueeze(item[batch_idx], dim=0) for item in box_cls_flattened
        ]
        element_box_regression = [
            torch.unsqueeze(item[batch_idx], dim=0) for item in box_regression_flattened
        ]
...
class RegionProposalNetworkWILDS(RegionProposalNetwork):
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        Returns:
            ...
            box_loss (Tensor)
        """
        objectness, pred_bbox_deltas = batch_concat_box_prediction_layers(
        ...
        for (
            ...
            objectness_,
            pred_bbox_deltas_,
        ) in zip(objectness, regression_targets, labels, objectness, pred_bbox_deltas):

            sampled_pos_inds, sampled_neg_inds = self.fg_bg_sampler(
                torch.unsqueeze(labels_, dim=0)
            )
...
class FastWILDS(GeneralizedRCNN):
    def __init__(
        ...
        image_mean = [
            ...
            0.0,
            0.0,
        ]  # small trick because images are already normalized
...
class FasterRCNNLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        try:
            elementwise_loss = torch.stack(
                [sum(v for v in item["losses"].values()) for item in outputs]
            )
        except:
