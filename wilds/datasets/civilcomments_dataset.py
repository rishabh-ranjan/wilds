...
class CivilCommentsDataset(WILDSDataset):
    ...
    def __init__(
        ...
        self._metadata_df = pd.read_csv(
            os.path.join(self._data_dir, "all_data_with_identities.csv"), index_col=0
        )
        ...
        self._metadata_array = torch.cat(
            (
                ...
                torch.LongTensor(
                    (self._metadata_df.loc[:, self._auxiliary_vars] >= 0.5).values
                ),
        ...
        self._eval_groupers = [
            CombinatorialGrouper(dataset=self, groupby_fields=[identity_var, "y"])
