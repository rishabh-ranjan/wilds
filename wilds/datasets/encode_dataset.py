...
def dnase_normalize(input_bw_celltype, ref_celltypes, out_fname, data_pfx):
    ...
    for the_chr in chrom_sizes:
        ...
        bw = pyBigWig.open(
            data_pfx + "DNASE.{}.fc.signal.bigwig".format(input_bw_celltype)
        )
...
class EncodeDataset(WILDSDataset):
    ...
    def __init__(
        ...
        official_val_cts = {"MAX": ["HepG2"], "JUND": ["HepG2"]}
        official_test_cts = {"MAX": ["liver"], "JUND": ["liver"]}
        ...
        if self._split_scheme == "official":
            splits = {
                "train": {"chroms": train_chroms, "celltypes": train_celltypes},
                "id_val": {"chroms": val_chroms, "celltypes": train_celltypes},
                "val": {"chroms": val_chroms, "celltypes": val_celltype},
                "test": {"chroms": test_chroms, "celltypes": test_celltype},
                "id_test": {"chroms": test_chroms, "celltypes": train_celltypes},
            }
            self._split_dict = {
                "train": 0,
                "val": 1,
            ...
            splits = {
                ...
                "val": {"chroms": val_chroms, "celltypes": test_celltype},
                "test": {"chroms": test_chroms, "celltypes": test_celltype},
            }
            self._split_dict = {
                "train": 0,
        ...
        # Add new split scheme specifying custom test and val celltypes in the format val.<val celltype>.test.<test celltype>, e.g. self._split_scheme == 'official' is equivalent to self._split_scheme == 'val.HepG2.test.liver'
            ...
            train_celltypes = [
                ct for ct in all_celltypes if ((ct != in_val_ct) and (ct != in_test_ct))
            ]
        ...
        self._metadata_df = pd.read_csv(
            self._data_dir
            + "/labels/{}/metadata_df.bed".format(self._transcription_factor),
            sep="\t",
            header=None,
            index_col=None,
            names=["chr", "start", "stop", "celltype"],
        )
        self._y_array = torch.tensor(
            np.load(
                self._data_dir
                + "/labels/{}/metadata_y.npy".format(self._transcription_factor)
            )
        )
        ...
        self._all_chroms = sorted(
            list({chrom for _, d in splits.items() for chrom in d["chroms"]})
        )
        self._all_celltypes = sorted(
            list({chrom for _, d in splits.items() for chrom in d["celltypes"]})
        )
        ...
        for ct in self._all_celltypes:
            orig_dnase_bw_path = os.path.join(
                self._data_dir, "DNASE.{}.fc.signal.bigwig".format(ct)
            )
            dnase_bw_path = os.path.join(
                self._data_dir,
                "DNase.{}.{}.{}.bigwig".format(
                    self._transcription_factor, ct, self._split_scheme
                ),
            )
            if not os.path.exists(dnase_bw_path):
                ref_celltypes = splits["train"]["celltypes"]
                dnase_normalize(
                    ct, ref_celltypes, out_fname=dnase_bw_path, data_pfx=self._data_dir
                )
        ...
        for ct in self._all_celltypes:
            if ct not in test_cts:
                self._norm_ref_distr += (1.0 / num_to_avg) * self._dnase_qnorm_arrays[
                    ct
                ]
        ...
        chr_ints = (
            self._metadata_df["chr"]
            .replace(dict([(y, x) for x, y in enumerate(self._metadata_map["chr"])]))
            .values
        )
        celltype_ints = (
            self._metadata_df["celltype"]
            .replace(
                dict([(y, x) for x, y in enumerate(self._metadata_map["celltype"])])
            )
            .values
        )
        self._metadata_array = torch.stack(
            (torch.LongTensor(chr_ints), torch.LongTensor(celltype_ints)), dim=1
        )

        self._eval_grouper = CombinatorialGrouper(
            dataset=self, groupby_fields=["celltype"]
        )
    ...
    def get_input(self, idx, window_size=12800):
        ...
        dnase_this = np.nan_to_num(
            dnase_bw.values(chrom, interval_start, interval_end, numpy=True)
        )
        return torch.tensor(np.column_stack([seq_this, dnase_this]).T)
