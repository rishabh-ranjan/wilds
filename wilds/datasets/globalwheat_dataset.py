...
class GlobalWheatDataset(WILDSDataset):
    ...
    def __init__(
        ...
        for split_name, split_idx in self._split_dict.items():
            ...
            all_boxes = [
                GlobalWheatDataset._decode_string(box_string)
                for box_string in boxes_string
            ]
            self._split_array.extend([split_idx] * len(all_boxes))

            labels = [
                {
                    "boxes": torch.stack([torch.tensor(box) for box in boxes]),
    ...
    def eval(self, y_pred, y_true, metadata):
        ...
        results_str = (
            f"Average detection_acc across session: {detection_acc_avg_dom:.3f}\n"
            + results_str
        )
    ...
    def _decode_string(box_string):
        ...
        if box_string == "no_box":
            ...
            try:
                boxes = np.array(
                    [
                        np.array([int(eval(i)) for i in box.split(" ")])
