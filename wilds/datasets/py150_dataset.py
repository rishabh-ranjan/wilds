...
class Py150Dataset(WILDSDataset):
    ...
    def __init__(
        ...
        _tok_type = torch.tensor(
            list(df["tok_type"].apply(lambda x: x[1:]).values)
        )  # [n_samples, seqlen-1]
    ...
    def _compute_acc(self, y_pred, y_true, eval_pos):
        ...
        assert (
            flattened_y_pred.size() == flattened_y_true.size()
            and flattened_y_pred.dim() == 1
        )
    ...
    def eval(self, y_pred, y_true, metadata, prediction_fn=None):
        ...
        tok_type = metadata[
            :, 1:-1
        ]  # [n_samples, seqlen-1]. must splice off coarse domain info at end.
        results = {}
        results_str = ""

        # Acc for class & method combined
        eval_pos = (tok_type == self._TYPE2ID["class"]) | (
            tok_type == self._TYPE2ID["method"]
        )
    ...
    def _load_all_data(self):
        ...
        def get_split_name(name):
            if name.startswith("OOD"):
                return name.replace("OOD", "")
            if name.startswith("ID"):
                return name.replace("ID", "id_")
            return name
        ...
        for type in ["train", "IDval", "OODval", "IDtest", "OODtest"]:
            ...
            fnames = open(
                self._data_dir / f"metadata/repo_file_names/{type}.txt"
            ).readlines()
            repo_ids = [fname2repo_id(fname, repo_name2id) for fname in fnames]
            splits = [get_split_name(type)] * len(inputs)
            tok_types = json.load(
                open(self._data_dir / f"processed/{type}_input_tok_type.json")
            )
