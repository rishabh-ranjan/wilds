...
class OGBPCBAUnlabeledDataset(WILDSUnlabeledDataset):
    ...
    def __init__(
        ...
        if version is not None:
            raise ValueError(
                "Versioning for Unlabeled MolPCBA is handled through the OGB package. Please set version=none."
            )
        ...
        meta_dict[
            "url"
        ] = "http://snap.stanford.edu/ogb/data/wilds/molpcba_unlabeled.zip"
        ...
        self.ogb_dataset = PygGraphPropPredDataset(
            name="molpcba_unlabeled", root=root_dir, meta_dict=meta_dict
        )
        ...
        metadata_file_path = os.path.join(
            self.ogb_dataset.root, "processed", "group_assignment.npy"
        )
        self._metadata_array = (
            torch.from_numpy(np.load(metadata_file_path)).reshape(-1, 1).long()
        )
